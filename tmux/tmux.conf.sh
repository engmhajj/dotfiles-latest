...
setw -g pane-base-index 1 # make pane numbering consistent with
set -g renumber-windows on

# Enables tracking of focus events, allows tmux to respond when the terminal
set-option -g focus-events on # window gains or looses focus
...
set -g pane-border-lines double # Border lines between panes are thicker
...
unbind Space
bind-key Space switch-client -l # Alternate session
...
###############################################################################
#NOTE: ThePrimeagen's tmux-sessionizer script, got 'em
###############################################################################
...
set -g @tmux2k-right-plugins "uptime battery time bandwidth"
...
set -g @tmux2k-session-colors "black red" # set cpu plugin bg to red, fg to black
...
# run '~/.tmux/plugins/tmux/plugins/tmux-plugins/tmux-battery/battery.tmux'
